...
//        String incorrectBookingsFilePath = args[3];
        ...
        if(handler != null) {
            ...
            FlightDetails.FlightDetailsHashMap flightDetailsHashMap = new FlightDetails.FlightDetailsHashMap();
            flightDetailsHashMap.initializeFlights(inputList);

            List<BookingRequest> bookingRequests = handler.handleBookingRequestsFile(bookingRequestsFilePath);

            List<BookingDetails> bookingDetails = flightDetailsHashMap.validateBooking(bookingRequests);
           ...
           List<String> invalidBookings= flightDetailsHashMap.getInvalidBookings();
